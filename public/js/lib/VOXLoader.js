/**
 * VOXLoader（MagicaVoxel .vox 格式加载器）— 本地化自 three r185 examples/jsm/loaders/VOXLoader.js
 * 2026-09-24：供 Three.js 代码块运行环境使用（网上代码 import { VOXLoader } 后经
 * autoDeclareImports 落到 THREE.VOXLoader）。
 * 挂载采用等待策略：THREE 未就绪时每 50ms 重试（最多 10s），任意加载顺序均安全。
 */
(function () {
  ...
  (function attach() {
    if (!window.THREE) {
      if (++__voxAttempts < 200) return setTimeout(attach, 50);
      console.warn('[VOXLoader] 等待 THREE 超时，VOXLoader 未注册');
      return;
    }
    const {
      BufferGeometry, Color, Data3DTexture, FileLoader, Float32BufferAttribute, Group,
      Loader, LinearFilter, Matrix4, Mesh, MeshStandardMaterial, NearestFilter, RedFormat, SRGBColorSpace
    } = window.THREE;
...
class VOXLoader extends Loader {
	...
	parse( buffer ) {
		...
		if ( Object.keys( nodes ).length > 0 ) {
			...
			// 本地化补丁（2026-09-24）：无节点图的老式 VOX 文件 scene 兜底
			scene = new Group();
			for ( let c = 0; c < chunks.length; c ++ ) scene.add( buildMesh( chunks[ c ] ) );
		}
...
class VOXData3DTexture extends Data3DTexture {
    ...
    console.log('[VOXLoader] 已挂载 THREE.VOXLoader（MagicaVoxel 格式支持）');
  })();
})();

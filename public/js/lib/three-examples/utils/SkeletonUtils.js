/* [本地 vendor 修改] 原上游为 `import {...} from 'three'`（需 importmap 指向 three-shim）。
   本页面 THREE 为全局 UMD（js/lib/three.min.js），且 three-shim 未导出 AnimationMixer/
   SkeletonHelper，故改为从 window.THREE 解构——与 FBXLoader 等 vendor 文件加载时机
   一致（three.min.js 同步先加载，动态 import 本模块时 window.THREE 必已就绪）。 */
const {
	AnimationClip,
	AnimationMixer,
	Matrix4,

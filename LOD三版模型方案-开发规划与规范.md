...
| 会话 3 机动 2b | 用户驳回"工具下限"说法，坚持**全部低模必须 ≤100 面**。新增第二级强制压缩：**新建** `src/services/glbFaceReducer.js`——对 gltfpack 输出做**贪心边坍缩**（最短边合并→删退化面→顶点紧缩重写 accessor/bufferView/BIN，纯 JS；输入为未压缩纯几何 GLB，nodes/materials JSON 不动→借材质链路不受影响；带贴图文件也接受，images 随 bufferView 搬运）。实测 315→98 面、幂等。接线：`_generateLow` 在 -sa 级联后若仍 >100 面自动调 reducer（剥贴图前移到 reducer 之前）；`lod_regen_low_100faces.js` 加 fixup 阶段（先剥贴图再补压）。**最终全库复核：118/118 全部 ≤100 面**（原 21 个超限文件全部补压成功：女生 168→98、课桌 402→84、鲁迅 263→98、红军全系 95~100 等）；唯一例外口径：`model-1788498327710`（当天新上传的 2481 mesh/97 材质碎片集合体）每个 primitive 已压至 1~2 面的下限，958 面是 primitive 数量地板，再压必须按材质合并 primitive（涉及借材质链路风险，待用户单独决策）。浏览器实渲染 98/84/95 面样本全部正常。验收：stage7 11/11 + stage7b 15/15 + stage6 22/22 | ✅（2026-09-13） |
...
| 脚本 | `scripts/lod_regen_low_100faces.js`（**新增**） | 会话 3 机动 2（全量重生成低模至 100 面标准，失败回滚；存量重跑幂等；fixup 阶段对超限文件先剥贴图再补压） |
| 后端 | `src/services/glbFaceReducer.js`（**新增**） | 会话 3 机动 2b（贪心边坍缩强制 ≤100 面：gltfpack 拓扑下限的第二级压缩，绝对面数保证） |

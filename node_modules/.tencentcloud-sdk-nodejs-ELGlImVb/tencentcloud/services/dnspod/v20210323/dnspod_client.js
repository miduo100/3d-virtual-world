...
备注：新添加的解析记录存在短暂的索引延迟，如果查询不到新增记录，请在 30 秒后重试
     ...
     * 批量修改域名递归解析加速状态
     */
    async ModifyDomainRecursiveStatusBatch(req, cb) {
...
2.  API获取的记录总条数会比控制台多2条，原因是： 为了防止用户误操作导致解析服务不可用，对2021-10-29 14:24:26之后添加的域名，在控制台都不显示这2条NS记录。
     ...
     * 批量修改域名CNAME加速状态
     */
    async ModifyDomainCNAMESpeedupStatusBatch(req, cb) {
...
备注：新添加的解析记录存在短暂的索引延迟，如果查询不到新增记录，请在 30 秒后重试
     ...
     * 批量修改记录
     */
    async ModifyRecordBatchV3(req, cb) {
        return this.request("ModifyRecordBatchV3", req, cb);
    }
    /**
     * 获取域名操作日志导出文件下载地址
     */
    async DescribeDomainLogFilterFile(req, cb) {
        return this.request("DescribeDomainLogFilterFile", req, cb);
    }
    /**
     * 套餐绑定、解绑、更换域名
     */
    async ModifyPackageDomain(req, cb) {

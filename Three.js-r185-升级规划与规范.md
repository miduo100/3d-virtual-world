...
| 阶段 5 专项回归 | ✅ 完成（验收通过） | **③骨骼动画套件回归：67/67 PASS**（`scripts/accept_stage5_skeleton.js`，可重跑）。覆盖 10 模板 = 5 大历史案（线上转换的GLB[B类双重轴] / 拿剑武士[副本骨骼链] / Kipfel_Mobile[裙子收起] / 谁到发疯[半身埋地+容器下沉] / zhu跳舞[mofx_rig 趴地案族]）+ 全部活模板（来跳舞/美女/新生成美女无皮/metool转GLB/测试跳跃不管用），每模板 6 判据：A glbModel 加载 / B sharedMixer 绑定当前模型 / C idle 运行 / D 轨道命中>0 / E 骨骼驱动 quatDelta>0 / F **蒙皮站立姿态（程序化新判据：SkinnedMesh.applyBoneTransform 采样顶点→世界包围盒，sizeY>sizeZ×1.15 不趴、minY>-0.5 不深埋、maxY>0.9 身高正常——比阶段 1 的 compDiag+目检更严格且全自动）**；L0 修复器 4 模块在场（REVISION=185 / dup / comp v2.1.0 / bone-physics）；幻影石 #482 不复发（DB 0 行 + 场景无 20260708 节点）；0 错误。谁到发疯专项 compDiag 与阶段 1 基线一致（matched=51/sunk/willCompensate/!restBroken）。**②顺手修复 2 个真 bug（均为 r185 升级暴露）**：(a) **线上转换的GLB 播放动画躺倒**（上会话遗留未固化，`_online_*.txt` 调试残留即此案）：animConventionCompensator 升级 v2.1.0 新增「C 类动画侧容器轴向补偿」——模型侧容器旋转下沉归一后，动画文件自身骨架容器（Armature+90°X 角色 GLB 转的动画）的同款旋转暴露，Hips 轨道呈「窄幅大偏移」（实测 86.3°±0.3°）→ 播放躺倒；修复=Hips 轨道整体左乘 corr = hipsBind × mean⁻¹（上会话 Node 已验证 lie→stand），触发判据全模板扫描校准（`scripts/verify_s5_hips_scan.mjs`）：仅 sunk 模型 + 偏移 min>60°（标准站姿偏移实测 43.3°，间隔充足）+ 带宽<12°（舞蹈动作带宽 170° 不误伤）；P/K 对 Hips 是恒等（归一后父世界=I、表内 Hips 世界=I、L=I）两层不冲突；Node 全 10 模板验证 `verify_s5_anim_axis.mjs` 10/10 PASS（线上转换GLB stand+axisFixed=76 帧，其余 9 模板零轴向干预不劣化）。(b) **_bakeSkinsToStatic（幻影石烘焙）r185 兼容**：原代码置 `node.skeleton=null` 后节点仍是 SkinnedMesh 类实例，r151+ 新增的 `SkinnedMesh.getVertexPosition`（raycast/computeBoundingBox 路径）调 applyBoneTransform 读 null.bones → 偶发 "Cannot read properties of null (reading 'bones')"（r128 无此方法所以旧版安全）；修复=保留 skeleton 引用（烘焙时骨骼处于 bind，boneMW×IBM=I，getVertexPosition=恒等变换，与"顶点固定 bind pose"语义完全一致），删置空一行，修复后验收 0 错误。**③FBX r184 up-axis 实测：14/14 PASS**（`scripts/accept_stage5_fbx.js`）：Mixamo FBX×2 套拿剑武士 GLB（mixamo 同约定标准模型），走真实链路（_loadPlayerAnimGlb→AnimLoader→r185 ESM FBXLoader），蒙皮盒站立（sizeY1.44-1.45>sizeZ1.07-1.15）、脊柱沿 +Y（Hips.y0.88→Spine.y0.92→Head.y1.28-1.31，Z 不变）、轨道命中 52/52、骨骼驱动、0 错误——r184 自动 +Z-up→+Y-up 转换正确无需补偿。截图 `Screenshot/accept_stage5/`（12 张：10 模板 + 2 FBX）。**④gaussianSplatRenderer shader 实测：26/26 PASS**（会话 10，`scripts/accept_stage5_3dgs.js`，可重跑）。先造样本：`scripts/gen_3dgs_test_ply.js` 生成 INRIA 标准 62 属性二进制小端 PLY（24000 点 = 彩虹球 18000[主导轴扇区着色：±X 红 / ±Y 绿 / ±Z 蓝高饱和大色块] + 灰色地面盘 6000；法线 + f_rest_0..8 非零 → 覆盖 loader 一阶 SH 展开分支；mulberry32 确定性）→ `public/scenes/3dgs/scene-1786835882322-897112501.ply`（5.68MB，恰为 test_gaussian.html 原引用文件名；页面硬编码"83.8MB"文案已改为通用文案）。L1 测试页 14/14：REVISION=185、WebGL2 上下文、加载✅/渐进 100%/FPS>0、**0 控台错误 = GLSL1 源码（attribute/varying/gl_FragColor）经 r185 WebGL2 自动转 GLSL3 编译通过**、红/绿/蓝扇区像素断言（13526/3454/13601 像素）黑盒验证协方差投影→gl_PointSize→高斯衰减→手动 pow(1/2.2) 全链路（**r152+ 自定义 ShaderMaterial 无 colorspace_fragment include 不做自动输出编码，shader 内手动 gamma 不构成双重编码，维持现状正确**）、dispose→重载→再可见。L2 主世界内 addGaussianSplat 9/9：真实生产渲染器上下文（precision:mediump + logarithmicDepthBuffer:true + sortObjects:false，与测试页不同的风险组合）下 splat 创建/可见/**renderer.info.render.points=24000（真实绘制计数，非仅节点在场）**/drawRange 全量（LOD 不误剪）/render 已被 world3dgs 包装/0 新增错误/前后截图 diff 0.4%+扇区色增量 4380 像素；走 unloadObject 真实删除路径清理。**⑤WebGL2 检测与友好降级：3/3 PASS**：新建 `public/js/webgl2Guard.js`（IIFE ≤100 行，不碰 299KB 黑名单 world.js）插入 index.html `three.min.js` 之前——支持 WebGL2 时零副作用（仅暴露 window.checkWebGL2Support 供复用）；不支持时 `window.__WEBGL2_UNSUPPORTED=true` + 全屏内联样式中文提示（浏览器升级建议 Chrome/Edge 79+/Firefox 51+ + 硬件加速开关指引 + 远程桌面提示）+ `window.stop()` 中止剩余解析（world.js 等后续脚本不执行，避免初始化抛错刷屏；注意浏览器预加载扫描器可能已预取 three.min.js 字节但**不会执行**，权威判据 window.THREE===undefined）。L3 验收（addInitScript stub `getContext('webgl2')→null` 模拟旧设备）：overlay 可见+文案正确+flag+three 未执行；正常路径（L2）确认 guard 零误触发。**验收方法论沉淀（会话 10）**：(a) 主世界 animate 循环每帧把相机重置回玩家后方，测试中强制摆相机无效——把测试对象放到「相机视线 unproject(0,-0.3,0.5) 射线 11m 处」才保证在屏内（NDC 断言 |x|,|y|<1）；(b) 玩家出生点在 y≈11 高台，世界内放 3DGS 用固定 y=0 会整球埋入地形；(c) BufferGeometry 只有 setDrawRange 方法，读取用 `geometry.drawRange.count` 属性（getDrawRange 不存在）；(d) position:fixed 元素 offsetParent 恒为 null，可见性断言用 getBoundingClientRect；(e) window.stop() 中止文档解析后 domcontentloaded 永不触发，playwright 须 waitUntil:'commit'+catch 超时。截图 `Screenshot/accept_stage5/` 新增 5 张（3dgs_l1_testpage / 3dgs_l1_reload / 3dgs_l2_world_before / 3dgs_l2_world_after / 3dgs_l3_webgl2_guard） | 下次入口：阶段 6 收尾（①删 `js/lib/_backup_r128/`【需用户确认】②提醒同步 ubuntu-deploy-package 部署包③重建 linux_node_modules.gz（含 three 0.185）④package.json 锁 0.185.x⑤本文档归档进 README 引用 + git diff 全量评审）。遗留观察项：全场景 raycast 偶发 null.matrixWorld（try/catch 内、未复现、pageerror=0），待观察；webgl2Guard 目前仅覆盖 index.html（用户主入口），admin 系编辑器页如需可复用 window.checkWebGL2Support 自行接入 |
| 阶段 6 收尾 | ✅ 完成 | **r185 升级工程全部收官（2026-09-04）**。①`js/lib/_backup_r128/` 已删除（11 文件，用户确认；该目录从未入 git，r128 原文件确认可从 `5bb9ef5`（Initial commit）恢复：`git cat-file -e 5bb9ef5:public/js/lib/three.min.js`）；②**`linux_node_modules.gz` 重建（43.1MB，旧包 47.8MB 是 2026-08-07 的升级前版本）**：临时目录 `npm install --os=linux --cpu=x64 --omit=dev --ignore-scripts --no-audit --no-fund` 生成 Linux 平台生产依赖（248 包/13525 条目）后 `tar -czf` 打包；验证通过：three 0.185.1 ✓、sharp 含 linux-x64/libvips-linux 二进制 ✓（**修复旧包缺 sharp 的口径缺陷**——旧包无 sharp，部署到 Linux 后纹理压缩服务实际是坏的）、gltfpack/tencentcloud 在场 ✓、playwright/esbuild 等开发依赖已排除 ✓；部署用法不变（`tar -xzf linux_node_modules.gz` 解压即用）；③package.json 版本锁定：`three: ^0.185.1 → ~0.185.1`、`@types/three: ^0.185.4 → ~0.185.4`（tilde 只允许 0.185.x 补丁位，杜绝意外跳 0.186），package-lock.json 同步；④README.md / README_CN.md 归档引用：徽章与技术栈 Three.js 0.183 → **0.185 + WebGL2**，Related Docs/相关文档表新增《Three.js-r185-升级规划与规范》入口；⑤git diff 全量评审通过（本阶段 5 文件：README×2 + package.json + package-lock.json + linux_node_modules.gz，无半成品无临时文件残留）。**同步提醒（外部动作，待用户执行）**：ubuntu-deploy-package 部署包不在本工作区，需用户手动同步本次升级的全部前端产物（public/js/lib/ 下 three.min.js 与 stub 化加载器、three-r185/、three-examples/、three-shim.js、webgl2Guard.js、各编辑器 HTML 改动）与新 linux_node_modules.gz | 升级工程完结。遗留观察项（非阻塞）：全场景 raycast 偶发 null.matrixWorld（try/catch 内未复现）待观察；webgl2Guard 仅覆盖 index.html，编辑器页可复用 window.checkWebGL2Support 自行接入；其他存量数据库部署需补 `ALTER TABLE weapons ADD COLUMN IF NOT EXISTS icon_emoji VARCHAR(10)` |

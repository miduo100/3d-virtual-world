...
- 若查询当天用量，由于统计延迟等原因，返回数据可能不够准确。
     */
    DescribeTRTCSegmentModerationUsage(req: DescribeTRTCSegmentModerationUsageRequest, cb?: (error: string, rep: DescribeTRTCSegmentModerationUsageResponse) => void): Promise<DescribeTRTCSegmentModerationUsageResponse>;
    /**
...
- 若查询当天用量，由于统计延迟等原因，返回数据可能不够准确。
     */
    DescribeTRTCDedicatedCloudAccUsage(req: DescribeTRTCDedicatedCloudAccUsageRequest, cb?: (error: string, rep: DescribeTRTCDedicatedCloudAccUsageResponse) => void): Promise<DescribeTRTCDedicatedCloudAccUsageResponse>;
    /**
     * 查询旁路转码计费时长。
- 查询时间小于等于1天时，返回每5分钟粒度的数据；查询时间大于1天时，返回按天汇总的数据。
- 单次查询统计区间最多不能超过31天。
...
- 若查询当天用量，由于统计延迟等原因，返回数据可能不够准确。
     */
    DescribeTRTCAIRecognitionUsage(req: DescribeTRTCAIRecognitionUsageRequest, cb?: (error: string, rep: DescribeTRTCAIRecognitionUsageResponse) => void): Promise<DescribeTRTCAIRecognitionUsageResponse>;
    /**
     * 查询先前注册的声纹信息
     */

...
本接口为异步接口，本接口返回成功后需以返回的 RequestID 为入参，调用 DescribeTaskStatus 接口查询本次任务是否成功。
     ...
     */
    async DisassociateBudget(req, cb) {
...
本接口为异步接口，本接口返回成功后需以返回的 RequestID 为入参，调用  [DescribeTaskStatus](https://cloud.tencent.com/document/product/214/30683)  接口查询本次任务是否成功。
     ...
     * 查询模型路由列表页
     */
    async DescribeModelRouters(req, cb) {
...
本接口为异步接口，接口返回成功后，需以得到的 RequestID 为入参，调用 [DescribeTaskStatus](https://cloud.tencent.com/document/product/214/30683) 接口查询本次任务是否成功。
     ...
     */
    async ModifyKeysUserGroup(req, cb) {
...
本接口为异步接口，本接口返回成功后需以返回的RequestID为入参，调用 [DescribeTaskStatus](https://cloud.tencent.com/document/product/214/30683) 接口查询本次任务是否成功。
     ...
     * 禁用/启用Key
     */
    async ModifyKeysBlockStatus(req, cb) {
...
本接口为异步接口，本接口返回成功后需以返回的RequestID为入参，调用 [DescribeTaskStatus](https://cloud.tencent.com/document/product/214/30683) 接口查询本次任务是否成功。
     ...
     * 删除模型路由实例
     */
    async DeleteModelRouters(req, cb) {
...
本接口为异步接口，本接口返回成功后需以返回的RequestID为入参，调用  [DescribeTaskStatus](https://cloud.tencent.com/document/product/214/30683)  接口查询本次任务是否成功。
     ...
     * 查询Budget列表。
     */
    async DescribeBudgets(req, cb) {
        return this.request("DescribeBudgets", req, cb);
    }
    /**
     * 批量创建Key
     */
    async CreateKeys(req, cb) {
     ...
     */
    async ModifyModelRouterAttributes(req, cb) {
...
本接口为异步接口，本接口返回成功后需以返回的RequestID为入参，调用  [DescribeTaskStatus](https://cloud.tencent.com/document/product/214/30683)  接口查询本次任务是否成功。
     ...
     */
    async DescribeBudgetAssociations(req, cb) {
...
本接口为异步接口，本接口返回成功后需以返回的RequestID为入参，调用 [DescribeTaskStatus](https://cloud.tencent.com/document/product/214/30683) 接口查询本次任务是否成功。
     ...
     * 创建模型路由实例
     */
    async CreateModelRouter(req, cb) {
     ...
     */
    async ModifyUserGroupAttributes(req, cb) {
        return this.request("ModifyUserGroupAttributes", req, cb);
    }
    /**
     * 查询模型路由详细信息
     */
    async DescribeModelRouterDetail(req, cb) {

...
export declare class Client extends AbstractClient {
     ...
     * 从环境池里立即取出1个环境
     */
    ReleaseEnv(req: ReleaseEnvRequest, cb?: (error: string, rep: ReleaseEnvResponse) => void): Promise<ReleaseEnvResponse>;
    /**
...
注意：本接口为只读 dry-run 操作，不落库、不创建任何资源，仅返回各项检查的详细结果。本接口通过不代表 CreateHTTPServiceRoute 必然成功（例如证书运行时状态、并发抢占等仍需创建时最终判定），但本接口不通过则 CreateHTTPServiceRoute 必然不通过。
     */
    VerifyHTTPServiceRoute(req: VerifyHTTPServiceRouteRequest, cb?: (error: string, rep: VerifyHTTPServiceRouteResponse) => void): Promise<VerifyHTTPServiceRouteResponse>;
    /**
...
修改云函数、云存储和数据库表的基础权限配置。支持预定义权限级别和自定义安全规则两种方式配置资源访问权限。
     */
    ModifyResourcePermission(req: ModifyResourcePermissionRequest, cb?: (error: string, rep: ModifyResourcePermissionResponse) => void): Promise<ModifyResourcePermissionResponse>;
    /**
...
解绑之后，会导致云存储不可用，请谨慎操作。
     */
    UnbindStorageSource(req: UnbindStorageSourceRequest, cb?: (error: string, rep: UnbindStorageSourceResponse) => void): Promise<UnbindStorageSourceResponse>;
    /**
     * 删除tcb用户
     */
     ...
     */
    ListPGUserMigrations(req: ListPGUserMigrationsRequest, cb?: (error: string, rep: ListPGUserMigrationsResponse) => void): Promise<ListPGUserMigrationsResponse>;
    /**
     * 检查是否开通Tcb服务
     */
     ...
     */
    DescribeCreditsUsageDetail(req: DescribeCreditsUsageDetailRequest, cb?: (error: string, rep: DescribeCreditsUsageDetailResponse) => void): Promise<DescribeCreditsUsageDetailResponse>;
    /**
...
限制：一个环境最大可允许加入20个认证源。
     ...
     * 查询应用服务信息
     */
    DescribeCloudAppInfo(req?: DescribeCloudAppInfoRequest, cb?: (error: string, rep: DescribeCloudAppInfoResponse) => void): Promise<DescribeCloudAppInfoResponse>;
    /**
     * 销毁云服务器实例
     */
...
每个环境仅允许绑定 1 个外部云存储源。
     */
    BindStorageSource(req: BindStorageSourceRequest, cb?: (error: string, rep: BindStorageSourceResponse) => void): Promise<BindStorageSourceResponse>;
    /**
     * 修改日志主题
     */
...
安全域名被删除之后，可能会引起跨域问题，请谨慎操作。
     ...
     * 查询资源点模式下的资源点用量
     */
    DescribeCreditsUsage(req: DescribeCreditsUsageRequest, cb?: (error: string, rep: DescribeCreditsUsageResponse) => void): Promise<DescribeCreditsUsageResponse>;
    /**
...
本接口仅更新存储源绑定关系，不会迁移您的数据。
     */
    ModifyStorageSource(req: ModifyStorageSourceRequest, cb?: (error: string, rep: ModifyStorageSourceResponse) => void): Promise<ModifyStorageSourceResponse>;
    /**
...
该接口跟 [DescribeTables](https://cloud.tencent.com/document/api/876/127962) 接口功能一致，后续该接口可能会下线，请使用 [DescribeTable](https://cloud.tencent.com/document/api/876/127962)接口。
     ...
     * 创建云应用
     */
    CreateCloudApp(req: CreateCloudAppRequest, cb?: (error: string, rep: CreateCloudAppResponse) => void): Promise<CreateCloudAppResponse>;
    /**
     * 本接口（DescribePGUserMigration）用于查询目标环境指定 migration 详情。
     */
    DescribePGUserMigration(req: DescribePGUserMigrationRequest, cb?: (error: string, rep: DescribePGUserMigrationResponse) => void): Promise<DescribePGUserMigrationResponse>;
    /**
...
开通托管模型前：通过本接口查询平台支持的托管模型及其规格，结合 [UpdateAIModel](https://cloud.tencent.com/document/product/876/131316) 接口完成模型配置。
     ...
     * 查询云应用服务列表信息
     */
    DescribeCloudAppList(req: DescribeCloudAppListRequest, cb?: (error: string, rep: DescribeCloudAppListResponse) => void): Promise<DescribeCloudAppListResponse>;
    /**
     * 创建虚拟服务器
...
接口入参中的 Tag 为文档型数据库的实例 Id，可以通过 [DescribeEnvs](https://cloud.tencent.com/document/api/876/34820) 接口返回的 EnvList[0].Databases[0].InstanceId 获取。
     ...
     */
    RepairPGUserMigrationHistory(req: RepairPGUserMigrationHistoryRequest, cb?: (error: string, rep: RepairPGUserMigrationHistoryResponse) => void): Promise<RepairPGUserMigrationHistoryResponse>;
    /**
...
查询云函数、云存储和数据库表的基础权限配置。支持单个资源查询和批量查询。
     */
    DescribeResourcePermission(req: DescribeResourcePermissionRequest, cb?: (error: string, rep: DescribeResourcePermissionResponse) => void): Promise<DescribeResourcePermissionResponse>;
    /**
     * 查询环境计费周期。
...
返回的 KeyID 和 PrivateKey 需与环境 ID 一起组装为 JSON 配置文件，供客户端 Admin SDK 初始化时使用，文件格式如下：
{
  ...
  "env_id": "<请求时传入的 EnvId>"
     ...
     * 删除云应用服务
     */
    DeleteCloudApp(req: DeleteCloudAppRequest, cb?: (error: string, rep: DeleteCloudAppResponse) => void): Promise<DeleteCloudAppResponse>;
    /**
     * 查询构建日志
     */
    DescribeCloudBaseRunBuildLog(req: DescribeCloudBaseRunBuildLogRequest, cb?: (error: string, rep: DescribeCloudBaseRunBuildLogResponse) => void): Promise<DescribeCloudBaseRunBuildLogResponse>;
    /**
     * 白名单接口，申请Tcb角色临时凭证
     */
    AssumeRoleForAllocatedEnv(req: AssumeRoleForAllocatedEnvRequest, cb?: (error: string, rep: AssumeRoleForAllocatedEnvResponse) => void): Promise<AssumeRoleForAllocatedEnvResponse>;
    /**
     ...
     * 查询云应用服务版本信息
     */
    DescribeCloudAppVersion(req: DescribeCloudAppVersionRequest, cb?: (error: string, rep: DescribeCloudAppVersionResponse) => void): Promise<DescribeCloudAppVersionResponse>;
    /**
     * 本接口（PreviewPGUserMigrations）用于预览SQL migrations 在远端的执行计划，不实际执行SQL。
     */
    PreviewPGUserMigrations(req: PreviewPGUserMigrationsRequest, cb?: (error: string, rep: PreviewPGUserMigrationsResponse) => void): Promise<PreviewPGUserMigrationsResponse>;
    /**
     * 查询服务器价格
     */
...
该接口支持自动扣除代金券（AutoVoucher=true时），符合条件的代金券会被自动扣除。
     ...
     * 从环境池里立即取出1个环境
     */
    AllocateEnv(req: AllocateEnvRequest, cb?: (error: string, rep: AllocateEnvResponse) => void): Promise<AllocateEnvResponse>;
    /**
...
该接口支持自动扣除代金券（AutoVoucher=true时），符合条件的代金券会被自动扣除。
     ...
     */
    PushPGUserMigrations(req: PushPGUserMigrationsRequest, cb?: (error: string, rep: PushPGUserMigrationsResponse) => void): Promise<PushPGUserMigrationsResponse>;
    /**
     * 搜索用户调用日志
     */
...
若自定义登录（CUSTOM）或邮箱登录（EMAIL）身份源尚不存在，调用该接口时将自动创建。
     ...
     * 获取云应用cos信息
     */
    DescribeCloudAppCosInfo(req: DescribeCloudAppCosInfoRequest, cb?: (error: string, rep: DescribeCloudAppCosInfoResponse) => void): Promise<DescribeCloudAppCosInfoResponse>;
    /**
     * 获取可售卖的新套餐列表，含详情，如果传了PackageId，则只获取指定套餐详情
     */
    DescribeEnvPlans(req: DescribeEnvPlansRequest, cb?: (error: string, rep: DescribeEnvPlansResponse) => void): Promise<DescribeEnvPlansResponse>;
    /**
     * 云服务器规格list
     */
     ...
     * 查询云应用服务版本列表信息
     */
    DescribeCloudAppVersionList(req: DescribeCloudAppVersionListRequest, cb?: (error: string, rep: DescribeCloudAppVersionListResponse) => void): Promise<DescribeCloudAppVersionListResponse>;
}
